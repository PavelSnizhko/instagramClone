...
class ApiUserService extends MetaApi {
  ...
  Future<User> getUser(int id) async {
    final response = await client.get("$baseUrl/api/users/$id/");
  ...
  Future<List<User>> getFriends(int id) async {
    final response = await client.get("$baseUrl/api/users/$id/friends");
    if (response.statusCode == 200) {
...
class APIFeedService extends MetaApi {
  Future<FeedModel> getFeedByUserId(int id) async {
    final response = await client.get("$baseUrl/api/feeds/$id/");
    if (response.statusCode == 200) {
